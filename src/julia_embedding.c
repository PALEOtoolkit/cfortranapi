...
static int load_julia_functions()
{
    ...
    if (!libjulia) {
        fprintf(stderr, "Failed to load libjulia - check LD_LIBRARY_PATH is set to include the directory with libjulia.so\n");
        return 0;
    }
...
void *get_cfunction_pointer(const char *name, int *success)
{
    void *p = 0;
...
int julia_initialize(const char *julia_system_image_path)
{
    if (!load_julia_functions())
        return 0;
